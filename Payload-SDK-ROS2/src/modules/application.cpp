...
#include "../../include/common/osal/osal.h"
#include "../../include/common/osal/osal_fs.h"
#include "../../include/common/osal/osal_socket.h"
#include "../modules/hal/hal_usb_bulk.h"
#include "../modules/hal/hal_uart.h"
#include "../modules/hal/hal_network.h"
...
#define DJI_LOG_PATH_MAX_SIZE           (256)
#define DJI_LOG_FOLDER_NAME_MAX_SIZE    (256)
#define DJI_SYSTEM_CMD_STR_MAX_SIZE     (256)
...
#define DJI_USE_SDK_CONFIG_BY_JSON      (1)
...
Application::Application(std::string config_json_path, std::string log_path): config_json_path_(config_json_path), log_path_(log_path)
{
...
void Application::DjiUser_SetupEnvironment()
{
    ...
    // printConsole.consoleLevel = DJI_LOGGER_CONSOLE_LOG_LEVEL_DEBUG;
    ...
    std::cout << "config_json_path_        == " << config_json_path_ << std::endl;
    std::cout << "config_json_path_.size   == " << config_json_path_.size() << std::endl;
    if (config_json_path_.size() > 1) {
        DjiUserConfigManager_LoadConfiguration(config_json_path_.c_str());
    } else {
    ...
    if (DjiUser_LocalWriteFsInit(log_path_) != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
...
void Application::DjiUser_ApplicationStart()
{
    ...
    returnCode = DjiCore_SetAlias("Skysys_Wind_PSDK");
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
...
T_DjiReturnCode Application::DjiUser_LocalWriteFsInit(std::string log_path)
{
    ...
    uint8_t ret;
    std::string path;
    std::string log_index_file_name;
    std::string lastest_log_path;

    path                = log_path + "DJI";
    log_index_file_name = log_path + "index";
    lastest_log_path    = log_path + "latest.log";

    printf("log_path: %s\n", log_path.c_str());
    printf("lastest_log_path: %s\n", lastest_log_path.c_str());

    printf("log_path length: %d\n", log_path.length());
    printf("lastest_log_path length: %d\n", lastest_log_path.length());
    ...
    if (access(log_path.c_str(), F_OK) != 0) {
        sprintf(folderName, "mkdir -p %s", log_path.c_str());
    ...
    s_djiLogFileCnt = fopen(log_index_file_name.c_str(), "rb+");
    if (s_djiLogFileCnt == nullptr) {
        s_djiLogFileCnt = fopen(log_index_file_name.c_str(), "wb+");
        if (s_djiLogFileCnt == nullptr) {
            return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
        }
    } else {
    ...
    sprintf(filePath, "%s_%04d_%04d%02d%02d_%02d-%02d-%02d.log", path.c_str(), currentLogFileIndex,
    ...
    if (logFileIndex >= DJI_LOG_MAX_COUNT) {
        sprintf(systemCmd, "rm -rf %s_%04d*.log", path.c_str(), currentLogFileIndex - DJI_LOG_MAX_COUNT);
    ...
    sprintf(systemCmd, "ln -sfrv %s %s", filePath, lastest_log_path.c_str());
    system(systemCmd);
